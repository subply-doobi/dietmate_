...
const logResult = (
  ...
  totalTryMenu.forEach((v, i) => {
    console.log(
      `menu ${String(i + 1).padEnd(5)} | ES: ${String(v.errScore).padEnd(
        5
      )} | 칼: ${String(v.nutr.calorie).padEnd(5)} 탄: ${String(
        v.nutr.carb
      ).padEnd(5)} 단: ${String(v.nutr.protein).padEnd(5)} 지: ${String(
        v.nutr.fat
      ).padEnd(5)} | 가격: ${String(v.price).padEnd(3)} | 카테고리: ${
